...
# Read Jackie data file (2025-01-03) and update table

```{r read-jackie-data, include=FALSE, echo=TRUE}
...
seq_technology_samples <- unique(df$seq_technology)
```

Single cell sequencing was done by using `r seq_unit_samples` and there are `r assay_samples` and `r seq_technology_samples` sequencing technologies in the database. Cohort is formed and processed accordingly. 
...
## Number of samples per experiment, tissue, seq_unit, assay, condition, and sample

```{r, fig.align = "left", results = "asis", message = FALSE, warning = FALSE, echo = FALSE}
tables1 <- df %>% count(experiment, tissue, seq_unit, assay, seq_technology, condition, Sample)
cat("  \n<div style=\"font-size:80%\">  \n")
caption_value = "Summary of samples per experiment, tissue, seq_unit, assay, seq_technology, condition, and Sample"
...
## Number of samples per experiment, tissue, seq_unit, assay, seq_technology, condition, and sample
