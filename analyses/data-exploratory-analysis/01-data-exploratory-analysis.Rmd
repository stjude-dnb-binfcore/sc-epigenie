...
This is an exploratory analysis of the data available for the testing phase for the `sc-atac-seq` pipeline(s). We are investigating number of samples overall per condition and variables as defined in the `params`.

We are looking for cohorts that fit the following criteria as described in the [./analyses/README.md](https://github.com/stjude-dnb-binfcore/sc-atac-seq/tree/main/analyses).
...
```{r read-updated-processed-data, include=FALSE, echo=TRUE}
...
df <- df_processed %>%
  filter(multiome_10x == "no",
         !matched_sample_info == "different-mouse-same-tissue-same-age")
```
...
## Number of samples per experiment, tissue, seq_unit, seq_technology_assay, condition, and sample

```{r, fig.align = "left", results = "asis", message = FALSE, warning = FALSE, echo = FALSE}
tables1 <- df %>% count(experiment, tissue, seq_unit, seq_technology_assay, condition, Sample)
cat("  \n<div style=\"font-size:80%\">  \n")
caption_value = "Summary of samples per experiment, tissue, seq_unit, seq_technology_assay, condition, and Sample"
