...
df_processed <- read_excel(jackie_data_file) %>%
  ...
  mutate(tissue = case_when(grepl("Retina", experiment) ~ "Retina",
         ...
         matched_sample_id = paste(Sample, assay, sep = "_")) %>%
...
Single cell sequencing was done by using `r seq_unit_samples` and there are `r seq_technology_assay_samples` sequencing technologies and assays in the database. Cohort is formed and processed accordingly. 
...
Here, we investigate the number of libraries per assay, i.e., `seq_technology_assay` and per `Sample`.

```{r, fig.align = "left", results = "asis", message = FALSE, warning = FALSE, echo = FALSE}
tables1 <- df %>% count(experiment, tissue, condition, Sample, seq_unit, seq_technology_assay)
cat("  \n<div style=\"font-size:80%\">  \n")
...
```{r, fig.align = "left", results = "asis", message = FALSE, warning = FALSE, echo = FALSE}
tables1 <- df %>% 
  #select(experiment, tissue, condition, Sample, seq_unit, seq_technology_assay, seq_technology_assay_sum) %>%
  select(experiment, tissue, condition, Sample, seq_unit, seq_technology_assay) %>%
  group_by() 
cat("  \n<div style=\"font-size:80%\">  \n")
