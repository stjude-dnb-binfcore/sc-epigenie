...
print(table(rna_reference@meta.data$DYE))
print(table(atac_query@meta.data$DYE))
...
# Create a new metadata column indicating high/low prediction score
atac_query$score_group <- ifelse(atac_query$prediction.score.max > 0.5, "Above 0.5", "0.5 or Below")

# Plot UMAP colored by score group
name <- paste0(plots_dir, "/", glue::glue("plot-umap-cells-prediction.score.max.png"))
p <- DimPlot(atac_query, reduction = "umap", group.by = "score_group") +
  scale_color_manual(values = c("Above 0.5" = "#1b9e77", "0.5 or Below" = "#d95f02")) +
  labs(title = "UMAP: Cells by Prediction Score Group") +
  theme_Publication()
print(p)
ggsave(file = name, plot = p, width = 8, height = 6, device = "png")
