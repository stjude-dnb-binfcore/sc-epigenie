...
Because the `links` table can be very large, only a subset of the most confident links is displayed in the HTML report to avoid slow loading or browser crashes. Links are ranked by co‑accessibility score (higher scores indicate stronger, more confident links), and only the top **~500 links** are shown for inspection. The full links table is preserved separately for downstream analysis.

```{r links-interactive-table, echo=TRUE}
table1 <- links_df %>% dplyr::arrange(desc(score)) %>%   # co-accessibility score
