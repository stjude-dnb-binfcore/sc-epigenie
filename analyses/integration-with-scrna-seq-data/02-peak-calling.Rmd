...
#matching_cells <- paste0(str_remove(grep(paste0("^", sample, ":"), colnames(seurat_obj), value = TRUE), pattern = paste0(sample,":")),"-1")
...
write.table(peaks, file = paste(results_dir, "/", "peaks.tsv", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE)
```
...
for (i in seq_len(nrow(top_markers))) {
  ...
  plot_path <- file.path(coverage_plots_dir, paste0("CoveragePlot_overall_", ct, "_", gene, ".png"))
...
```{r coverage-plot-split-by-id, fig.width = 10, fig.height = 15, fig.fullwidth = TRUE, echo=TRUE}
for (i in seq_len(nrow(top_markers))) {
  gene <- top_markers$gene[i]
  ct <- top_markers$cluster[i]
  ...
  # Save the plot as PNG
  plot_path <- file.path(coverage_plots_dir, paste0("CoveragePlot_ID_", ct, "_", gene, ".png"))
...
## Plotting genomic regions across cell types: Shared

### Overall

```{r coverage-plot-shared-peaks-overall, fig.width = 10, fig.height = 6, fig.fullwidth = TRUE, echo=TRUE}
...
for (region_to_plot in top_n_peaks) {
  ...
  print(p)
  # Save plot
  plot_path <- file.path(coverage_plots_dir, paste0("CoveragePlot_all_cell_types_shared_genes_overall_", region_to_plot, ".png"))
  ggsave(plot_path, plot = p, width = 10, height = 6)
...
```{r coverage-plot-shared-peaks-split-by-id, fig.width = 10, fig.height = 15, fig.fullwidth = TRUE, echo=TRUE}
# If `markers` contains DE peaks across cell types (from Signac peak assay)
shared_peaks <- markers %>%
  group_by(gene) %>%
  summarize(n_groups = n_distinct(cluster)) %>%
...
for (region_to_plot in top_n_peaks) {
  ...
  print(p)
  # Save plot
  plot_path <- file.path(coverage_plots_dir, paste0("CoveragePlot_all_cell_types_shared_genes_ID_", region_to_plot, ".png"))
  ggsave(plot_path, plot = p, width = 10, height = 15)
