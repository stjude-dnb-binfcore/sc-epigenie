...
for (sample in sample_name) {
  # Subset to cells from this sample
  sample_cells <- grep(paste0("^", sample, ":"), colnames(seurat_obj), value = TRUE)
  seurat_obj_sample <- subset(seurat_obj, cells = sample_cells)
  
  # Find markers within this sample
  ...
  all_markers[[sample]] <- markers
}

# Combine results for all samples
markers_by_sample <- do.call(rbind, all_markers)
...
for (sample in sample_name) {
  # Subset the Seurat object for the current sample
  sample_cells <- grep(paste0("^", sample, ":"), colnames(seurat_obj), value = TRUE)
  seurat_obj_sample <- subset(seurat_obj, cells = sample_cells)
  ...
  for (i in seq_len(nrow(top_markers))) {
    ...
    message("Plotting ", gene, " for cell type ", ct, " in sample ", sample)
    
    p <- CoveragePlot(
      object = seurat_obj_sample,
      region = gene,
      group.by = "predicted.id",
      idents = ct,
      ranges = peaks,
    ...
    # Save the plot as PNG, including sample name in the filename
    plot_path <- file.path(coverage_plots_dir, paste0("CoveragePlot_", sample, "_", ct, "_", gene, ".png"))
    ggsave(plot_path, plot = p, width = 10, height = 6)
