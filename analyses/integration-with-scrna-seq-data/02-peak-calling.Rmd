...
##############################################################
# We need to rename the fragments files to match the naming in the cell barcodes of the seurat_obj
# or the CallPeaks function wont detect any peaks due to the naming mismatch
##############################################################
# Skip if final processed files already exist
...
if (all_files_exist) {
  message("All processed fragment files and their indexes already exist. Skipping renaming and re-indexing step.")
} else {
  ##############################################################
  # Rename barcodes in fragment files and re-compress
  for (i in seq_along(sample_name)) {
    ...
    if (!file.exists(backup_fragments_gz)) {
      ...
      message(paste("Backup created for sample:", sample))
    } else {
      message(paste("Backup already exists for sample:", sample))
    }

    # Unzip, modify, and rewrite
    gunzip(fragments_gz, remove = FALSE, overwrite = TRUE)
    dt <- data.table::fread(fragments_tmp, header = FALSE, sep = "\t")
    dt[[4]] <- paste0(sample, ":", gsub("-1$", "", dt[[4]]))
    dt <- dt[, 1:5]
    data.table::fwrite(dt, fragments_tmp, sep = "\t", col.names = FALSE)
    R.utils::gzip(fragments_tmp, destname = fragments_gz, overwrite = TRUE)

    message(paste0("Processed fragments for sample: ", sample))
  }
...
fragment_list <- mapply(function(sample, frag_path) {
  # CORRECTED: use `sample`, not `sample_name`
...
Next, we scale the accessibility data for the selected features and generate a heatmap showing the accessibility profiles across all cells. These peaks are the most enriched in accessibility per predicted.id and can highlight regulatory elements associated with specific cell states.
...
## Use shared marker peaks across cell types
