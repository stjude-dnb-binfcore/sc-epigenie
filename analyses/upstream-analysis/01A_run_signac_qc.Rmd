...
source(paste0(analysis_dir, "/util/function-create-vlnplots-variable.R"))
```
...
- **Ratio reads in peaks promoters**: Promoter regions represent the proximal regulatory elements of genes, typically defined as windows surrounding transcription start sites (TSS). A high proportion of reads mapping to promoter peaks indicates strong accessibility signal at gene promoters, which is often associated with well-positioned transcriptional regulation. Conversely, very low promoter accessibility relative to total peak counts may reflect poor data quality or cell states with globally reduced promoter activity. 

    - Interpretation guidance: While the exact range is experiment-dependent, cells with extremely low fractions (e.g., <5–10%) may represent low-quality libraries or non-regulatory fragments. Cells with abnormally high fractions may reflect technical artifacts or over-restriction of accessible regions to promoters.

- **Ratio reads in peaks enhancers**: Enhancer regions are distal regulatory elements that contribute to context-specific gene regulation. Reads mapping to enhancer peaks provide information about cell type–specific regulatory programs and can complement promoter accessibility profiles. A balanced proportion of enhancer accessibility is expected in high-quality single-cell chromatin data, depending on the cell type and assay. Excessively low fractions may suggest limited capture of distal elements, while unusually high fractions may reflect biased enrichment. 

    - Interpretation guidance: Cells with near-zero enhancer fractions often indicate poor sensitivity or data quality. On the other hand, enhancer fractions that dominate total peak accessibility (e.g., >80–90%) may signal technical bias or contamination. Typical distributions vary by tissue and cell type, so thresholds should be chosen relative to dataset-specific quality profiles.
...
p <- print(create_vlnplots_variable(seurat_obj   = seurat_obj,
                                    features     = c("nCount_peaks", "nFeature_peaks", "TSS.enrichment", "nucleosome_signal", "blacklist_ratio"),
                                    out_path     = name,
                                    split_variable = NULL,         # or e.g., "ID" to split by sample/group
                                    pt.size      = 0.1,
                                    ncol         = 5,
                                    sample_title   = glue::glue("Before-filtering: {sample_name[i]}")))  # top-left title
ggsave(file = name, width = 15, height = 6, device = "png")
```

```{r vln-plot-before-filtering-2, fig.width = 25, fig.height = 6, fig.fullwidth = TRUE, echo=TRUE}
name <- paste0(plots_dir, "/", glue::glue("04-vln-plot-before-filtering-2.png"))
p <- print(create_vlnplots_variable(seurat_obj   = seurat_obj,
                                    features     = c("pct_reads_in_peaks", "peak_region_fragments", "mitochondrial", "duplicate", "pct_reads_in_peaks_promoters", "pct_reads_in_peaks_enhancers"),
                                    out_path     = name,
                                    split_variable = NULL,         # or e.g., "ID" to split by sample/group
                                    pt.size      = 0.1,
                                    ncol         = 6,
...
p <- print(create_vlnplots_variable(seurat_obj   = seurat_obj,
                                    ...
                                    pt.size      = 0.1,
                                    ncol         = 5,
                                    sample_title   = glue::glue("After-filtering: {sample_name[i]}")))  # top-left title
ggsave(file = name, width = 25, height = 6, device = "png")
```

```{r vln-plot-after-filtering-2, fig.width = 25, fig.height = 6, fig.fullwidth = TRUE, echo=TRUE}
