...
chrom_assay <- CreateChromatinAssay(counts = counts, sep = c(":", "-"), genome = genome_name, fragments = fragments_file, min.cells = 10, min.features = min.features_value) 
...
p <- print(VlnPlot(object = seurat_obj,
                   ...
                   pt.size = 0.1, ncol = 6) + plot_annotation(title = glue::glue("Before-filtering: {sample_name[i]}")))
...
p <- print(VlnPlot(object = seurat_obj,
        features = c("pct_reads_in_peaks", "peak_region_fragments", "mitochondrial", "duplicate",
                     "pct_reads_in_peaks_promoters", "pct_reads_in_peaks_enhancers"),
        pt.size = 0.1, ncol = 6) + plot_annotation(title = glue::glue("After-filtering: {sample_name[i]}")))
ggsave(file = name, width = 25, height = 6, device = "png")
```
