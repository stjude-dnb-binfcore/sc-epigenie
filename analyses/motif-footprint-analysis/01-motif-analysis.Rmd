...
This notebook guides users in performing DNA sequence motif analysis in Signac. We will explore two complementary options for performing motif analysis: one by finding overrepresented motifs in a set of differentially accessible peaks, one method performing differential motif activity analysis between groups of cells.
...
Idents(object = seurat_obj) <- "predicted.id"
...
# Use p_val_adj instead of raw p_val for more robust filtering
...
```{r top-da-peaks-select, echo=TRUE}
top_da_peaks <- top_da_peaks %>%
  group_by(cluster) %>% 
  top_n(n = top_n_value_motifs, wt = avg_log2FC)
```

### Overall

```{r plot-RunChromVAR-overall, fig.width = 25, fig.height = 5, fig.fullwidth = TRUE, echo=TRUE}
for (i in seq_len(nrow(top_da_peaks))) {
  gene <- top_da_peaks$gene[i]
  ct <- top_da_peaks$cluster[i]
  
  message("Plotting ", gene, " for cell type ", ct)
  p2 <- FeaturePlot(object = seurat_obj,
                  features = gene,
  ...
  plot_path <- file.path(ChromVAR_plots_dir, paste0("ChromVAR_da_peaks_top_overall_", ct, "_", gene, ".png"))
  ggsave(plot_path, plot = p, width = 15, height = 5)
  message("Plotting ", gene, " for cell type ", ct, ": Complete")
}
```

### Split by ID

```{r plot-RunChromVAR-ID, fig.width = 25, fig.height = 5, fig.fullwidth = TRUE, echo=TRUE}
for (i in seq_len(nrow(top_da_peaks))) {
  gene <- top_da_peaks$gene[i]
  ct <- top_da_peaks$cluster[i]
  ...
  p <- print(p1 + p2)
  # Save the plot as PNG
  plot_path <- file.path(ChromVAR_plots_dir, paste0("ChromVAR_da_peaks_top_ID_", ct, "_", gene, ".png"))
  ggsave(plot_path, plot = p, width = 25, height = 5)
  message("Plotting ", gene, " for cell type ", ct, ": Complete")
}
```
