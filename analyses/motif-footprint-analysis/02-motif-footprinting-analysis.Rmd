...
data_dir <- file.path(module_analysis_dir, "results", "01_motif_analysis") 

# Input files
data_file <- file.path(data_dir, "seurat_chromvar.rds")
...
First, we will use the seurat object as generated from the pipeline in the `r data_dir` directory. 
...
Idents(seurat_obj) <- seurat_obj@meta.data[["project"]]
...
footprinting_result <- add_footprinting(seurat_obj = seurat_obj, 
                                        ...
                                        motifs_to_fp_module_list = motifs_to_fp_module_list) 

# Extract and Save the objects
seurat_obj <- footprinting_result$seurat_obj 
saveRDS(seurat_obj, file = paste0(results_dir, "/", "seurat_obj_motifs_footprinting.rds")) # Save object

motif_obj <- footprinting_result$motif_obj 
...
We will group the footprint signal by aggregating accessibility across all cells in the dataset. We will plot shows the global footprint signal for each motif.
Each line in the plot represents one aggregated footprint curve per motif, providing a dataset‑wide view of motif accessibility. It is useful for a global view of motif accessibility without subgroup differences.

```{r plot-footprint-overall, fig.width = 35, fig.height = 20, fig.fullwidth = TRUE, echo=TRUE}
# No grouping by cell type, cluster, or metadata should be applied here.
# --- a) Plot ALL selected motifs together ---
p_all <- print(PlotFootprint(seurat_obj, features = motifs_to_fp))
ggsave(file.path(Footprinting_plots_dir, "Footprint_top_motifs.png"), plot = p_all, width = 35, height = 20)
```

```{r plot-footprint-per-motif, fig.width = 8, fig.height = 5, fig.fullwidth = TRUE, echo=FALSE, eval=FALSE}
...
```{r plot-footprint-celltype, fig.width = 35, fig.height = 20, fig.fullwidth = TRUE, echo=TRUE}
...
ggsave(file.path(Footprinting_plots_dir, "Footprint_top_motifs_celltype.png"), plot = p_celltype, width = 35, height = 20)
```
...
```{r plot-footprint-ID, fig.width = 35, fig.height = 25, fig.fullwidth = TRUE, echo=TRUE}
# --- c) Plot selected motifs together per cell type ---
p_celltype <- print(PlotFootprint(seurat_obj, features = motifs_to_fp, group.by = cell_type_name, 
                            split.by = "ID"))
ggsave(file.path(Footprinting_plots_dir, "Footprint_top_motifs_celltype_ID.png"), plot = p_celltype, width = 35, height = 25)
```
...
```{r plot-footprint-condition-value1, fig.width = 35, fig.height = 20, fig.fullwidth = TRUE, echo=TRUE}
...
ggsave(file.path(Footprinting_plots_dir, "Footprint_top_motifs_celltype_condition_value1.png"), plot = p, width = 35, height = 20)
```
...
```{r plot-footprint-condition-value2, fig.width = 35, fig.height = 20, fig.fullwidth = TRUE, echo=TRUE}
if (!is.null(condition_value2)) {
  ...
  ggsave(file.path(Footprinting_plots_dir, "Footprint_top_motifs_celltype_condition_value2.png"), plot = p, width = 35, height = 20)
}
```
...
```{r plot-footprint-condition-value3, fig.width = 35, fig.height = 20, fig.fullwidth = TRUE, echo=TRUE}
if (!is.null(condition_value3)) {
  ...
  ggsave(file.path(Footprinting_plots_dir, "Footprint_top_motifs_celltype_condition_value3.png"), plot = p, width = 35, height = 20)
}
```
