...
Here, we chose not to use `GeneActivity()` because it makes several assumptions that may not align with our specific analysis requirements. For example, `GeneActivity()`:

- Uses a default promoter window (typically ±2 kb around the TSS) for peak-to-gene mapping
- Relies on a fixed reference genome and gene annotation, which may not match the custom annotations we're using (e.g., from Ensembl or GENCODE)

While `GeneActivity()` is automated and convenient for quick analyses, it offers limited flexibility. In contrast, our manual approach allows for:
...
peak2gene <- sparseMatrix(
  i = queryHits(overlaps),
  j = subjectHits(overlaps),
  dims = c(length(peaks_gr), length(anno))
)
...
While the `GeneActivity()` function is available for this purpose, we are intentionally omitting its use in this section.

```{r compute-add-gene-activities-automated, echo=TRUE}
...
```{r add-gene-activities, echo=TRUE}
# Optionally: add gene activity matrix as a new assay to your Seurat object
