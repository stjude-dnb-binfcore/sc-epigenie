...
sample_name <- unique(as.character(seurat_obj$ID))
...
Because `fc_df_combined` can be very large, printing the full table in the HTML report may significantly slow loading or cause the page to crash. To ensure report stability, only a subset of the most informative rows (i.e., **500 top peaks by absolute avg_log2FC fold change**) is displayed in the report. The complete, unfiltered table is always saved to disk for downstream analysis. 

```{r fold-change-interactive-table, echo=TRUE}
table1 <- fc_df_combined %>% dplyr::arrange(desc(abs(avg_log2FC))) %>%
...
For more information, see [Plotting genomic regions](https://stuartlab.org/signac/articles/pbmc_vignette#plotting-genomic-regions), [Visualization vignette](https://stuartlab.org/signac/articles/visualization), and [Tutorial using DA peaks](https://www.youtube.com/watch?v=pRi4IEb361w).

```{r coverage-plot-fold-change, fig.width = 15, fig.height = 8, fig.fullwidth = TRUE, echo=TRUE, eval=FALSE}
# Loop over clusters
...
```{r coverage-plot-da-peaks, fig.width = 15, fig.height = 8, fig.fullwidth = TRUE, echo=TRUE}
# Loop over clusters
Idents(seurat_obj) <- use_res_col
clusters <- levels(seurat_obj)

# Add feature column to top_n da_peaks data frame
top_n_value$feature <- top_n_value$gene
# Plot top da_peaks
export_cluster_peak_plots(seurat_obj = seurat_obj, fc_df_combined = top_n_value, top_n = top_n_value_peaks, min.cutoff_value = min.cutoff_value, plots_dir = coverage_plots_dir, clustering_column = use_res_col)
``` 
