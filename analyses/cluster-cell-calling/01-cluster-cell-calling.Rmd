---
title: "Cluster cell calling analysis for sc-ATAC-Seq Analysis in 10X Genomics data"
...
This notebook is tasked to calculate clusters across the project. For more information, see [Signac clustering](https://stuartlab.org/signac/articles/mouse_brain_vignette).
...
if (data_dir_module == "integrative-analysis"){
  ...
  data_dir <- file.path(root_dir, "analyses", data_dir_module, "results", glue::glue('03_Filter_object{suffix}'))
...
In scATAC-seq analysis with Signac, clustering is performed using a graph-based approach, following the construction of a low-dimensional representation of the data via Latent Semantic Indexing (LSI). This method captures patterns in chromatin accessibility by performing singular value decomposition (SVD) on a term frequency–inverse document frequency (TF-IDF) matrix derived from peak-by-cell accessibility counts.

Seurat (via Signac) then applies a shared nearest neighbor (SNN) graph-based clustering approach, similar to strategies used in single-cell transcriptomics [Macosko et al., 2015](https://www.cell.com/fulltext/S0092-8674(15)00549-8). The clustering method builds upon earlier graph-based techniques such as [SNN-Cliq, Xu and Su, Bioinformatics, 2015](https://academic.oup.com/bioinformatics/article/31/12/1974/214505) and CyTOF data [PhenoGraph, Levine et al., Cell, 2015](https://pubmed.ncbi.nlm.nih.gov/26095251/), where each cell is embedded in a K-nearest neighbor (KNN) graph based on similarity in their LSI space representation. Edges in the graph are weighted using Jaccard similarity, measuring the overlap between each cell’s neighborhood.

This process is implemented using the [FindNeighbors()](https://satijalab.org/seurat/reference/findneighbors) function, with the first 2–30 LSI components typically used to define the neighborhood structure. It's important to avoid using the first LSI component (LSI 1), which often captures sequencing depth rather than biological variation—use dims = 2:30 instead.

To perform clustering, Signac uses modularity optimization algorithms, such as the Louvain algorithm (default) or the more refined [SLM algorithm, Blondel et al., 2008](https://arxiv.org/abs/0803.0476). This is executed via the [FindClusters()](https://satijalab.org/seurat/reference/findclusters) function. The resolution parameter controls clustering granularity:

   - Lower values (e.g., 0.2–0.4) yield fewer, broader clusters
   - Higher values (e.g., 1.0–1.4) generate more fine-grained clusters

Typical resolution settings for ~3,000 cells range between 0.4 and 1.2, though this should be tuned based on dataset size and expected heterogeneity. You can inspect cluster assignments using the [Idents()](https://satijalab.github.io/seurat-object/reference/Idents.html) function.
...
if (data_dir_module == "upstream-analysis"){
  ...
  emb <- Embeddings(seurat_obj, "LSI")
  
  # Add to metadata with exact column names
  seurat_obj@meta.data$LSI_1 <- emb[, 1]
  seurat_obj@meta.data$LSI_2 <- emb[, 2]
  } else {}
...
if (length(resolution_list) > 1 | is.null(resolution_list)) {
  ...
  print(clustree_overlay(seurat_obj, prefix = glue::glue("{assay}_snn_res."), x_value = "LSI_1", y_value = "LSI_2",
